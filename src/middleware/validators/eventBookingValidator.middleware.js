...
exports.createEventBookingSchema = [
    ...
    body(EventBookingModel.BookingSeats.as)
        .exists()
        ...
        .bail(),
    body(`${EventBookingModel.BookingSeats.as}.*.person_name`)
        .trim()
        .exists()
        ...
        .isAlpha('en-US', { ignore: ' ' })
        .withMessage('Must be alphabetic'),
    body(`${EventBookingModel.BookingSeats.as}.*.seat_number`)
        .exists()
        .withMessage('Seat number is required for each seat')
        .bail()
        .isInt()
        .withMessage('Should be an integer'),
    body(`${EventBookingModel.BookingSeats.as}.*.seat_row`)
        .trim()
        .exists()
        ...
        .withMessage('Must be alphabetic')
        .toUpperCase(),
    body(`${EventBookingModel.BookingSeats.as}.*.identification_number`)
        .trim()
        .exists()
    ...
    body(EventBookingModel.BookingParkingSpaces.as)
        .optional()
        .isArray()
        .withMessage('Booking parking spaces must be an array like [{space_number: 1, space_row: "A", p_floor_id: 1},{...}]')
        .bail(),
    body(`${EventBookingModel.BookingParkingSpaces.as}.*.space_number`)
        .exists()
        .withMessage('Space number is required for each parking')
        .bail()
        .isInt()
        .withMessage('Should be an integer'),
    body(`${EventBookingModel.BookingParkingSpaces.as}.*.space_row`)
        .trim()
        .exists()
        ...
        .withMessage('Must be alphabetic')
        .toUpperCase(),
    body(`${EventBookingModel.BookingParkingSpaces.as}.*.p_floor_id`)
        .trim()
        .exists()
